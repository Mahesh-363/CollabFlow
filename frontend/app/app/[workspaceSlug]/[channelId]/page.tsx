...
import { MessageSkeleton, ErrorState } from '@/components/LoadingState'
...
export default function ChannelPage() {
  ...
  const { data: msgData, isLoading, isError: msgError, refetch } = useQuery({
  ...
  const renderMessages = () => {
    ...
    if (msgError) return <ErrorState onRetry={refetch} />
  ...
  return (
    <div style={{ display: 'flex', height: '100vh', overflow: 'hidden', background: 'var(--bg-base)' }}>
      <Sidebar
        ...
        workspaceColor={workspaceData?.icon_color || 'var(--brand)'}
      />
      ...
      {showMembers && (
        <div style={{
          ...
          flexDirection: 'column', height: '100vh', overflow: 'hidden auto'
        }}>
          <div style={{
            ...
            {(members || []).map((m: any) => (
              <div key={m.id} style={{
                ...
                <div style={{
                  ...
                  background: m.user?.avatar_color || 'var(--brand)',
