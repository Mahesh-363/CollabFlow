...
export function Sidebar({ workspaceSlug, workspaceName, workspaceColor }: SidebarProps) {
  ...
  const statusColor: Record<string, string> = { online: '#10b981', away: '#f59e0b', dnd: '#ef4444', offline: '#6b7280' }

  return (
    <aside style={{ width: 260, minWidth: 260, height: '100vh', background: 'var(--bg-surface)', borderRight: '1px solid var(--border)', display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
      {/* Header */}
      <div style={{ padding: '13px 14px', borderBottom: '1px solid var(--border)', display: 'flex', alignItems: 'center', gap: 9, cursor: 'pointer' }} onClick={() => window.location.href = '/app'}>
        <div style={{ width: 30, height: 30, borderRadius: 8, background: workspaceColor || 'var(--brand)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 800, fontSize: 13, color: 'white', flexShrink: 0 }}>
          {workspaceName?.[0]?.toUpperCase()}
        </div>
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{ fontWeight: 700, fontSize: 13, color: 'var(--text-primary)', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{workspaceName}</div>
      ...
      <div style={{ flex: 1, overflow: 'hidden auto', padding: '8px' }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '6px 10px 4px' }}>
          <span style={{ fontSize: 10, fontWeight: 700, color: 'var(--text-muted)', textTransform: 'uppercase', letterSpacing: '.06em' }}>Channels</span>
          <button onClick={() => setShowCreate(true)} style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'var(--text-muted)', fontSize: 16, lineHeight: 1 }}>+</button>
        </div>
          ...
          return (
            <Link key={ch.id} href={`/app/${workspaceSlug}/${ch.id}`} className={`sidebar-item ${isActive ? 'active' : ''}`} style={{ justifyContent: 'space-between', fontSize: 13 }}>
              ...
              {ch.unread_count > 0 && !isActive && (
                <span style={{ background: 'var(--brand)', color: 'white', borderRadius: 99, fontSize: 10, padding: '1px 5px', fontWeight: 700, flexShrink: 0 }}>{ch.unread_count}</span>
              )}
            </Link>
          )
        })}
        {(channels as any[]).length === 0 && (
          <div style={{ padding: '8px 12px', fontSize: 12, color: 'var(--text-muted)', fontStyle: 'italic' }}>No channels yet</div>
        )}
      </div>
      ...
      <div style={{ padding: '8px 10px', borderTop: '1px solid var(--border)', display: 'flex', alignItems: 'center', gap: 8 }}>
        <div style={{ position: 'relative', flexShrink: 0 }}>
          ...
          <div style={{ position: 'absolute', bottom: -1, right: -1, width: 9, height: 9, borderRadius: '50%', background: statusColor[myStatus], border: '1.5px solid var(--bg-surface)' }} />
        </div>
        <div style={{ flex: 1, minWidth: 0 }}>
      ...
      {showCreate && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.6)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 100, backdropFilter: 'blur(4px)' }}>
          <div style={{ background: 'var(--bg-surface)', border: '1px solid var(--border)', borderRadius: 16, padding: 28, width: '100%', maxWidth: 400 }}>
            <h2 style={{ fontWeight: 700, fontSize: 18, color: 'var(--text-primary)', marginBottom: 20 }}>Create a channel</h2>
            <div style={{ marginBottom: 14 }}>
              <label style={{ display: 'block', marginBottom: 6, fontSize: 13, fontWeight: 500, color: 'var(--text-secondary)' }}>Name</label>
              <input className="input-field" placeholder="e.g. announcements" value={newCh.name} onChange={e => setNewCh({ ...newCh, name: e.target.value.toLowerCase().replace(/\s+/g, '-') })} />
            </div>
            <div style={{ marginBottom: 14 }}>
              <label style={{ display: 'block', marginBottom: 6, fontSize: 13, fontWeight: 500, color: 'var(--text-secondary)' }}>Description</label>
              <input className="input-field" placeholder="What is this channel about?" value={newCh.description} onChange={e => setNewCh({ ...newCh, description: e.target.value })} />
            </div>
            <div style={{ marginBottom: 20 }}>
              <label style={{ display: 'block', marginBottom: 6, fontSize: 13, fontWeight: 500, color: 'var(--text-secondary)' }}>Type</label>
            ...
            <div style={{ display: 'flex', gap: 10 }}>
              ...
              <button className="btn-primary" style={{ flex: 2 }} disabled={!newCh.name.trim() || createChannel.isPending} onClick={() => createChannel.mutate(newCh)}>
